...
def readme(text):
    ...
    text = once(text, "## Ponto exato de retomada\n\nRetomar no **Ciclo de Vida e Estados Funcionais da Capacidade 01 — Captura de Contexto**.\n\nPróxima entrega:\n\n- criar `PAS-001-CC-LIFECYCLE-001`;\n- consolidar o Registro de Captura de Contexto;\n- definir estados, transições, autorizações, contestação e encerramento;\n- preparar eventos, integrações e contrato final;\n- preservar o `PAS-001` em `0.5.0` até a auditoria final.", "## Ponto exato de retomada\n\nRetomar na **Edição Consolidada e Federada do PAS-001 — Guivos Journey 1.0.0**.\n\nPróxima entrega:\n\n- criar a edição candidata federada;\n- incorporar mapa, perguntas, supersessão e autoridade da auditoria;\n- validar links, versões, navegação e regressões;\n- preservar extensões especializadas;\n- manter o `PAS-001` vigente em `0.5.0` até `Ready for publication`.", "readme historical resumption")
    text = once(text, "As Capacidades 02 a 09 estão funcionalmente concluídas. `PAS-001-RECON-001 1.0.0` registra que a Capacidade 01 permanece `Substantially complete` e bloqueia o avanço direto para `PAS-001 1.0.0`.", "As Capacidades 01 a 09 estão funcionalmente concluídas. `PAS-001-AUDIT-001 1.0.0` autoriza a edição candidata consolidada e federada, sem autorizar publicação automática.", "readme product engineering")
    text = once(text, "| 01 — Captura de Contexto | Substantially complete |", "| 01 — Captura de Contexto | Functionally complete — 100% |", "readme map")
    text = once(text, "- [Reconciliação e Prontidão do Guivos Journey](docs/product-architecture/pas-001-guivos-journey-reconciliacao-fechamento.md)", "- [Reconciliação e Prontidão do Guivos Journey](docs/product-architecture/pas-001-guivos-journey-reconciliacao-fechamento.md)\n- [Ciclo de Vida da Captura de Contexto](docs/product-architecture/pas-001-captura-de-contexto-ciclo-de-vida.md)\n- [Eventos e Integrações da Captura de Contexto](docs/product-architecture/pas-001-captura-de-contexto-eventos-integracoes-funcionais.md)\n- [Contrato Final da Captura de Contexto](docs/product-architecture/pas-001-captura-de-contexto-kpis-cenarios-contrato-final.md)\n- [Auditoria Final do PAS-001](docs/product-architecture/pas-001-guivos-journey-auditoria-final-prontidao.md)", "readme links")
    return text
