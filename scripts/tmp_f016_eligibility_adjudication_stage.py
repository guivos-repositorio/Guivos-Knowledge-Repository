...
path = "README.md"
text = read(path)
text = replace_once(text, "| GKR-STATE-001 | **3.8.0** |", "| GKR-STATE-001 | **3.9.0** |", "README state version")
text = replace_once(text, "O [Roadmap 13.8.0](docs/roadmap.md)", "O [Roadmap 13.9.0](docs/roadmap.md)", "README roadmap")
write(path, text)
replace_section(path, "Próximo gate da auditoria", """`F-006` está formalmente `RESOLVED`. O eixo corrente é `F-016 — desmaterialização documental repo-wide`.
...
path = "docs/index.md"
text = read(path)
text = replace_once(text, "`GKR-STATE-001` **3.8.0**", "`GKR-STATE-001` **3.9.0**", "docs index state")
text = replace_once(text, "O [Roadmap 13.8.0](roadmap.md)", "O [Roadmap 13.9.0](roadmap.md)", "docs index roadmap")
write(path, text)
replace_section(path, "Próximo movimento", """O próximo eixo prioritário permanece `F-016 — desmaterialização documental repo-wide`.
...
text = read(path)
text = replace_once(text, "version: 3.8.0", "version: 3.9.0", "uxa index version")
text = replace_once(text, "GKR-STATE-001\n→ 3.8.0", "GKR-STATE-001\n→ 3.9.0", "uxa index state block")
write(path, text)
replace_section(path, "Estado sincronizado", """| Elemento | Estado |
|---|---|
...
text = read(path)
text = replace_once(text, "version: 3.8.0", "version: 3.9.0", "state version")
write(path, text)
replace_section(path, "2. Estado executivo", """```text
ERA
→ GE-2 — KNOWLEDGE
...
""")
replace_section(path, "26. Auditoria integral do corpus — estado corrente", """`GKR-FULL-CORPUS-AUDIT-001 v1.9.0` está ativo como instrumento temporário de execução.
...
""")
replace_section(path, "29. Próximo ato governado", """O Bloco 2 G/H/I está auditado/remediado com `F-006 RESOLVED`. O eixo corrente é `F-016`.
...
text = read(path)
text = replace_once(text, "version: 1.8.0", "version: 1.9.0", "audit version")
text = replace_once(text, "| H — Organização / Coletivo | `AUDITED / UPDATE_APPLIED / F-006_OPEN` | Jobs + IA propagados; F-006 cleanup-eligible, remoção física depende de autorização separada |", "| H — Organização / Coletivo | `AUDITED / UPDATE_APPLIED / F-006_RESOLVED` | Jobs + IA propagados; cleanup F-006 concluído e validado |", "audit H row")
text = replace_once(text, "| I — Registries / Catálogos / SVGs | `AUDITED / UPDATE_APPLIED / F-006_OPEN` | inventário físico provado; F-007 resolvido; cleanup/recontagem dependem de autorização separada |", "| I — Registries / Catálogos / SVGs | `AUDITED / UPDATE_APPLIED / F-006_RESOLVED / F-007_RESOLVED` | inventário pós-F-006 = 119 SVGs; F-016-A cleanup-eligible, remoção física ainda não autorizada |", "audit I row")
text = replace_once(text, "H/I. O/C + inventário visual                     [auditados/remediados; F-006 aberto e cleanup-eligible]", "H/I. O/C + inventário visual                     [auditados/remediados; F-006 resolvido]", "audit order H/I")
text = replace_once(text, "J/K/L/M/N. domínios especializados               [não liberados até decisão de fechamento G/H/I]", "F-016. desmaterialização documental              [aberta; F-016-A cleanup-eligible / physical cleanup not authorized]\n↓\nJ/K/L/M/N. domínios especializados               [não liberados automaticamente]", "audit order F016")
write(path, text)
replace_section(path, "22. Estado atual", """```text
AUDIT
→ IN_PROGRESS
...
""")
text = read(path)
anchor = "Critério de encerramento: **o GKR não pode competir com Design na definição de interface**."
addition = """Critério de encerramento: **o GKR não pode competir com Design na definição de interface**.
...
text = replace_once(text, anchor, addition, "audit F016 anchor")
write(path, text)

# Roadmap
path = "docs/roadmap.md"
text = read(path)
text = replace_once(text, "id: ROADMAP-13.8.0", "id: ROADMAP-13.9.0", "roadmap id")
text = replace_once(text, "version: 13.8.0", "version: 13.9.0", "roadmap version")
write(path, text)
replace_section(path, "1. Função", """Este roadmap traduz `GKR-STATE-001 v3.9.0` em **frentes governadas de avanço**.
...
""")
replace_section(path, "2. Baseline governada", """| Elemento | Estado vigente |
|---|---|
| Era | **GE-2 — Knowledge** |
...
""")
replace_section(path, "12. Lote H — Organização e Coletivo", """Estado do Bloco 2:
...
""")
replace_section(path, "13. Lote I — Registries, catálogos e materializações", """Estado do Bloco 2:
...
""")
replace_section(path, "26. Regra do próximo movimento", """`F-006` está encerrado. O próximo eixo governado é `F-016`.
...
text = read(path)
text = replace_once(text, "version: 1.0.0", "version: 1.1.0", "specialized homes version")
text = replace_once(text, "last_updated: 2026-08-29", "last_updated: 2026-09-05", "specialized homes date")
write(path, text)
replace_section(path, "2. Estado global preservado", """```text
AUDITORIA INTEGRAL
→ IN_PROGRESS
...
```
""")
replace_section(path, "11. Gate vigente", """```text
LOTE F
→ COMPLETED DOCUMENTALLY
...
NEXT PHYSICAL GATE
→ SEPARATE EXPLICIT HUMAN AUTHORIZATION FOR F-016-A
...
text = read(path)
text = replace_once(text, "version: 1.1.0", "version: 1.2.0", "derived audit version")
text = replace_once(text, "last_updated: 2026-08-30", "last_updated: 2026-09-05", "derived audit date")
write(path, text)
replace_section(path, "2. Baseline reconciliada", """```text
UXA-015..018
...
""")
text = read(path)
text = replace_once(text, "| `GKR-JOURNEY-SCREEN-CATALOG-001` | total canônico `121 / 121 validados` | 121 = total físico; maturidade por item; claim antiga superseded | normalizado |", "| `GKR-JOURNEY-SCREEN-CATALOG-001` | total canônico `121 / 121 validados` | **119 físicos pós-F-006**; maturidade por item; claim antiga superseded | normalizado |", "derived catalog row")
text = replace_once(text, "| `GKR-JOURNEY-SCREEN-GALLERY-001` | leitura agregada de validação | 121 SVGs físicos para inspeção; não 121 wireframes vigentes | normalizado |", "| `GKR-JOURNEY-SCREEN-GALLERY-001` | leitura agregada de validação | **119 SVGs físicos pós-F-006**; inventário ≠ autoridade visual | normalizado |", "derived gallery row")
text = replace_once(text, "| `GKR-JOURNEY-SCREEN-TRACEABILITY-MATRIX-001` | associação visual podia ser lida como vigência | 121 associações / 34 perfis; associação ≠ autoridade visual | normalizado |", "| `GKR-JOURNEY-SCREEN-TRACEABILITY-MATRIX-001` | associação visual podia ser lida como vigência | **119 associações / 34 perfis estáveis**; associação ≠ autoridade visual | normalizado |", "derived matrix row")
write(path, text)
replace_section(path, "11. Gate para normalização e cleanup", """Após o fechamento de F-006, qualquer derivado deve:
...
""")
replace_section(path, "13. Estado final desta auditoria derivada", """```text
PR #313
...
text = read(path)
text = replace_once(text, "version: 1.5.0", "version: 1.6.0", "EA index version")
text = replace_once(text, "last_updated: 2026-08-30", "last_updated: 2026-09-05", "EA index date")
write(path, text)
replace_section(path, "3. Cobertura visual e granular", """As contagens abaixo separam **inventário físico**, **associação documental** e **maturidade vigente**. A existência de arquivo ou associação não promove autoridade visual.
...
text = read(path)
text = replace_once(text, "version: 0.41.0", "version: 0.42.0", "journeys index version")
text = replace_once(text, "last_updated: 2026-08-30", "last_updated: 2026-09-05", "journeys index date")
write(path, text)
replace_section(path, "4. Estado documental", """| Camada | Estado | Referência |
|---|---|---|
...
# Assertions on current-authority surfaces only.
current_files = [
    "README.md",
    "docs/index.md",
...
for rel in current_files:
    content = read(rel)
    if "F-006 OPEN" in content or "F-006_OPEN" in content:
        raise SystemExit(f"STALE_F006_OPEN_REMAINS={rel}")
...
for rel in [
    ...
    if "3.9.0" not in read(rel):
        raise SystemExit(f"MISSING_3_9_PROPAGATION={rel}")

if "CLEANUP_ELIGIBILITY_PROVEN" not in read("docs/project/gkr-full-corpus-audit.md"):
    raise SystemExit("F016A_ELIGIBILITY_NOT_RECORDED")

print("F016_ELIGIBILITY_ADJUDICATION_TRANSFORM=PASS")
