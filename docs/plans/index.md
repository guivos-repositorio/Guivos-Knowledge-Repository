---
id: GKR-PLANS-INDEX-001
title: Planos — Visão Geral
status: active
version: 1.2.0
owner: Guivos
last_updated: 2026-09-20
normative: false
depends_on:
  - GEM-004-PLAN-TAXONOMY-AUTHORITY-001
  - GEM-004-A1
  - GEM-COMMERCIAL-BASELINE-001
  - GPA-004
---

# Planos — Visão Geral

Esta seção organiza os planos vigentes por contexto, com leitura separada entre **Pessoa**, **Coletivo**, **Organização** e **Guivos Business**.

Ela é uma superfície de consulta. As autoridades econômicas e funcionais de origem continuam prevalecendo em caso de divergência.

## Pessoa

Planos individuais para ampliar profundidade, personalização e capacidade da própria Journey.

| Plano | Mensal | Anual | Leitura principal |
|---|---:|---:|---|
| **Free** | R$ 0,00 | R$ 0,00 | começar sem barreira econômica |
| **Plus** | R$ 24,90 | R$ 249,00 | mais contexto, continuidade e personalização |
| **Pro** | R$ 49,90 | R$ 499,00 | maior profundidade analítica e integração |

[Ver detalhes dos planos de Pessoa](person.md)

## Coletivos

Planos para presença, mobilização, programas recorrentes e operação em rede.

| Plano | Mensal | Anual | Leitura principal |
|---|---:|---:|---|
| **Livre** | R$ 0,00 | R$ 0,00 | presença e mobilização inicial |
| **Mobiliza** | R$ 89,90 | R$ 899,00 | operação recorrente e monetização |
| **Impacta** | R$ 249,90 | R$ 2.499,00 | programas estruturados e mensuração ampliada |
| **Rede** | sob consulta | contrato anual | operação multiunidade e capacidade dimensionada |

[Ver detalhes dos planos de Coletivos](collectives.md)

## Organizações

Planos do participante institucional **Organização** dentro do ecossistema.

| Plano | Mensal | Anual | Leitura principal |
|---|---:|---:|---|
| **Conecta** | R$ 299,00 | R$ 2.990,00 | presença institucional e conexão estruturada |
| **Eleva** | R$ 799,00 | R$ 7.990,00 | maior recorrência, coordenação e analytics |
| **Transforma** | a partir de R$ 1.990,00 | contrato anual | operação institucional complexa e dimensionada |

[Ver detalhes dos planos de Organizações](organizations.md)

## Guivos Business

Planos do produto especializado B2B **Guivos Business**. Eles são independentes dos planos de Organização.

| Plano | Mensal | Anual | Leitura principal |
|---|---:|---:|---|
| **Start** | R$ 299,00 | R$ 2.990,00 | operar |
| **Growth** | R$ 799,00 | R$ 7.990,00 | acompanhar e compreender |
| **Scale** | a partir de R$ 1.990,00 | contrato anual | interpretar e integrar em escala |
| **Enterprise** | sob consulta | contrato anual | governança e operação de alta complexidade |

**Contratação:** online.

**Implementação/operação:** **Self-service quando possível**, com apoio do suporte quando necessário e operação gerenciada quando a complexidade exigir.

O modelo de implementação/operação é independente do plano: a complexidade da configuração determina o nível de acompanhamento.

[Ver detalhes dos planos do Guivos Business](business.md)

## Como ler os preços

Os valores acima representam a **baseline comercial de referência vigente no GKR**.

Eles não constituem, por si só:

- autorização automática de cobrança;
- oferta pública irrestrita;
- condição fiscal definitiva;
- promessa de capacidade infinita;
- SLA ou entitlement não formalizado.

Para planos sujeitos a dimensionamento, o preço final depende da capacidade contratada e das condições comerciais aplicáveis.

## Separações obrigatórias

```text
ORGANIZAÇÃO
= participante institucional
= Conecta · Eleva · Transforma

GUIVOS BUSINESS
= produto especializado B2B
= Start · Growth · Scale · Enterprise
```

Não existe equivalência automática:

```text
Organização Conecta ≠ Business Start
Organização Eleva ≠ Business Growth
Organização Transforma ≠ Business Scale ou Enterprise
```

## Fontes de autoridade

- `GEM-004-PLAN-TAXONOMY-AUTHORITY-001` — nomes, função e leitura conceitual;
- `GEM-004-A1` — preços, benefícios e limites de referência;
- `GEM-COMMERCIAL-BASELINE-001` — revisão da baseline comercial;
- `GPA-004` e `GPA-004-FUNCTIONAL-PORTFOLIO-001` — Guivos Business.
