...
Ele não substitui as autoridades normativas de cada domínio. Sua função é declarar, de forma curta e inequívoca, qual etapa está concluída, qual frente está ativa, quais frentes estão pausadas e qual próximo incremento está autorizado.
...
|---|---|
| Era | `GE-2 — Knowledge` |
| Marco vigente | `M7.3.1 — Repository State Precedence and Global Roadmap Reconciled` |
| Frente de controle | `GKR-REMEDIATION-002`; R1 e R2 concluídos, R3 a R5 pendentes |
| Frente arquitetural preservada | `A2-R03 — Business Architecture Review` |
| Execução da A2-R03 | temporariamente pausada até a reconciliação dos controles, navegação e validação mecânica |
...
→ reconciliação do estado e da navegação do GKR
...
Market Validation pode avançar sem substituir a prioridade arquitetural, por meio de:
...
A execução desses entregáveis exige incremento próprio. Este registro apenas preserva a autorização de coexistência entre a trilha operacional de evidência e a remediação documental.

## 7. Próximo incremento autorizado

Executar `R3 — Controles centrais`, seguido de `R4 — Navegação` e `R5 — Validação mecânica` do `GKR-REMEDIATION-002`.

Somente após o fechamento das não conformidades Major e resultado `PASS` da auditoria de correção poderá ocorrer `R6 — Retomada governada`, com retorno ao `BA-STR-002-CODR-001` e decisão individual sobre `ECO-CAND-003`.
