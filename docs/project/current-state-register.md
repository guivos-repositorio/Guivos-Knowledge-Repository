...
|---|---|
| Era | `GE-2 — Knowledge` |
| Marco vigente | `M7.8.1 — Seventh Human Outcome Decision Submitted` |
...
| COR | `0.18.0`; 16 `Under Validation`, 1 `Merged` e 1 `Rejected` |
| CODR | `0.12.0`; 6 de 18 decisões humanas; 1 submissão aguardando resposta |
| Submissão vigente | `BA-STR-002-COD-SUB-007 — ECO-CAND-007` |
...
| `ECO-CAND-007` | `Under Validation`; recomendação `Reformulate` submetida, sem decisão humana |
...
`COD-001` a `COD-006` permanecem registrados. A sexta decisão preservou `ECO-CAND-006` em `Under Validation` com a formulação candidata **Saúde relacional no ecossistema**.

## 4. Submissão vigente — ECO-CAND-007

A COEM recomenda `Reformulate` para **Participação inclusiva e digna**.
...
A submissão não cria `COD-007`, não altera o COR e não aprova o candidato.
...
→ decisões humanas do BA-STR-002 — 6 de 18; sétima submissão aberta
...
Nenhuma reformulação será executada automaticamente. `COD-007` somente poderá nascer após manifestação explícita do Fundador.
...
- registrar `COD-007` sem decisão humana;
- alterar `ECO-CAND-007` no COR;
