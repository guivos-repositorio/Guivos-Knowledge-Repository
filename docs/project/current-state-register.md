...
|---|---|
| Era | `GE-2 — Knowledge` |
| Marco vigente | `M7.10.1 — Ninth Human Outcome Decision Submitted` |
...
| CODR | `0.16.0`; 8 de 18 decisões humanas; 1 submissão aguardando resposta |
| Submissão vigente | `BA-STR-002-COD-SUB-009 — BUS-CAND-001` |
...
| `BUS-CAND-001` | `Under Validation`; recomendação `Reject` submetida, sem decisão humana |
...
`COD-001` a `COD-008` permanecem registrados. A oitava decisão preservou `ECO-CAND-008` em `Under Validation` com a formulação candidata **Participação protegida, justa e contestável**.

## 4. Submissão vigente — BUS-CAND-001
...
A recomendação retira apenas a candidatura do futuro catálogo de Business Outcomes. O conteúdo permanece proposto como princípio constitucional, obrigação de governança, critério de admissibilidade e referência para prevenção e correção de *mission drift*.

A submissão não cria `COD-009`, não altera o COR, não rejeita o candidato e não reduz a autoridade do propósito.
...
→ decisões humanas do BA-STR-002 — 8 de 18; nona submissão aberta
...
Nenhuma rejeição será executada automaticamente. `COD-009` somente poderá nascer após manifestação explícita do Fundador.
...
- registrar `COD-009` sem decisão humana;
- alterar `BUS-CAND-001` no COR;
