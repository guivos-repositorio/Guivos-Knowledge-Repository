...
`GIA-COG-001 v0.1.2` é a **Cognitive Reference Architecture vigente, ativa e normativa**, subordinada à GIA por `ADR-008`. Esse estado não autoriza `GIA-COG-002..008`, arquitetura física, dados reais, implementação, operação ou produção.
...
O estado ativo e normativo de `GIA-COG-001` não seleciona mecanismo, fornecedor ou topologia física.
...
→ GKR-UX-HOMES-DESIGN-DELIVERY-001 v7.0.45
