---
id: GPD-000
title: Documentação Pública da Guivos
status: active
version: 4.3.0
owner: Guivos
last_updated: 2026-09-09
related:
  - GOG-001
  - GKR-STATE-001
  - GKR-P9-GLOBAL-CONSOLIDATION-001
  - GKR-BRAND-SIGNATURE-001
  - GKR-BRAND-PUBLIC-AUTHORITY-001
  - GKR-BRAND-PUBLIC-AUTHORITY-PROPAGATION-001
  - GKR-GLOBAL-INTEGRITY-POST300-001
normative: false
---

# Documentação Pública da Guivos

Este domínio contém a tradução institucional pública das autoridades vigentes no Guivos Knowledge Repository.

## Public Canon vigente

- [GOG-001 — Guia Oficial da Guivos](guia-oficial-da-guivos.md) — **versão 5.3.0**.

`GOG-001` é a única superfície deste domínio classificada como `public-canon`.

## O que o Public Canon vigente consolida

- Pessoa, Coletivo e Organização como três participantes estruturais;
- sete Produtos Especializados: Journey, Mall, Travel, Business, Media, Intelligence e Ads;
- separação explícita `Organização ≠ Guivos Business`;
- taxonomia atual de planos;
- `Guivos Mall` como nome canônico;
- propósito antes de incentivos/pontos/créditos;
- Neo4j como tecnologia de referência, não implantação comprovada;
- Fundação Guivos como conceito institucional, não entidade jurídica presumida;
- privacidade por arquitetura sem presumir controles operacionais;
- Portugal/Lisboa como direção internacional candidata, não mercado ativo;
- fronteira entre maturidade documental e implementação real;
- autoridade verbal institucional `Possibility, lived.` / `Possibilidade, vivida.` e `#PossibilityLived` conforme `GKR-BRAND-SIGNATURE-001`;
- separação entre a assinatura institucional da Guivos e a assinatura pessoal do fundador conforme `GKR-BRAND-PUBLIC-AUTHORITY-001`.

## Regra editorial

```text
visão ≠ disponibilidade
arquitetura ≠ implementação
candidato ≠ aprovado/operacional
plano ≠ fato realizado
assinatura canônica ≠ marca registrada
fundador ≠ marca
assinatura pessoal do fundador ≠ assinatura institucional da Guivos
```

O Public Canon não pode elevar um objeto acima do estado evidenciado no GKR.

## Estrutura pública da Guivos

### Participantes

- Pessoa;
- Coletivo;
- Organização.

### Produtos Especializados

- Guivos Journey;
- Guivos Mall;
- Guivos Travel;
- Guivos Business;
- Guivos Media;
- Guivos Intelligence;
- Guivos Ads.

Produtos não são participantes nem pessoas jurídicas por definição.

## Autoridade verbal pública

```text
Assinatura global   Possibility, lived.
Versão PT           Possibilidade, vivida.
Hashtag global      #PossibilityLived
```

`Do possível ao vivido.` não integra a autoridade verbal institucional da Guivos. A expressão constitui assinatura pessoal e autoral de Guilherme Oliveira, fundador e principal referência humana pública inicial da Guivos, conforme `GKR-BRAND-PUBLIC-AUTHORITY-001`.

```text
GUIVOS ≠ FUNDADOR
DO POSSÍVEL AO VIVIDO. → FUNDADOR
POSSIBILITY, LIVED. → GUIVOS
POSSIBILIDADE, VIVIDA. → GUIVOS
```

A presença pública do fundador não transforma toda fala pessoal ou autoral em posição institucional da Guivos.

## Fundamento Cristão e fronteira pública

`GKR-CHRISTIAN-FOUNDATION-001` é autoridade fundacional normativa de **uso interno de governança**. Sua acessibilidade no GKR público não o promove automaticamente a Public Canon, copy institucional, campanha, produto ou comunicação religiosa externa.

Lucas 2:52 integra deliberadamente a bio pública pessoal do fundador por autoridade própria. Essa decisão não transforma o versículo em copy institucional obrigatória da Guivos nem em representação isolada de toda a base bíblica convergente da organização.

## Estado de maturidade

O marco funcional documental permanece **M7.88 / UXA-101**. UXA-102/V5 não foi iniciada e Engenharia de Produto permanece pausada antes de W0-01.

A publicação do Guia Oficial vigente não afirma que toda arquitetura documentada esteja implementada ou em produção.
