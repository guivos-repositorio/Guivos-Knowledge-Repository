...
7. registrar versão do instrumento, exclusões, limitações e possíveis vieses.
...
Os ajustes podem envolver narrativa, público prioritário, proposta de valor, situação de primeiro uso, escopo inicial, barreiras, monetização, canais de aquisição ou expansão geográfica.

### 3.3 Pivot parcial

A dor existe, porém uma parte relevante da solução, narrativa, público, mecanismo de valor, situação de uso ou contribuição esperada precisa mudar.
...
|---|---:|---:|---:|
| Dor relevante — Q8 | >= 65% | 50% a 64,9% | < 50% |
| Esforço médio — Q9 | >= 6,0 | 5,0 a 5,9 | < 5,0 |
| Compreensão autodeclarada — Q10 | >= 8,0 | 7,0 a 7,9 | < 7,0 |
...
| Intenção positiva de experimentar — Q17 | >= 60% | 45% a 59,9% | < 45% |
| Interesse confirmado na primeira experiência — Q18 | >= 35% | 20% a 34,9% | < 20% |

A disposição para pagar da `Q20` será analisada separadamente e não definirá, isoladamente, a decisão da primeira rodada.

Situação de primeiro uso (`Q13`), expectativas sobre a plataforma (`Q14`), resultado concreto de valor (`Q15`) e barreira principal (`Q19`) deverão orientar a interpretação e os ajustes, mas não possuem meta universal única nesta rodada.
...
- a dor for relevante, mas compreensão, aplicação prática, relevância, contribuição ou intenção estiverem abaixo do necessário;
...
Relevância ou contribuição abaixo de 7,0 deverão gerar, no mínimo, investigação de pivot. A classificação final dependerá da combinação com dor, compreensão e intenção.
...
3. Intenção declarada não equivale a adoção comprovada.
4. Contribuição percebida não equivale a transformação real.
5. Interesse na primeira experiência deve ser comparado com conversão em contato e participação efetiva.
6. Disposição para pagar deve ser validada posteriormente por comportamento de compra.
7. Situações de primeiro uso e resultados concretos devem orientar o recorte do MVP.
8. Preocupações espontâneas com privacidade ou confiança devem ser registradas, mas não transformadas em gate operacional antes de existir experiência demonstrável.
9. Médias gerais não devem ocultar segmentos ou estados com resultados muito diferentes.
10. Resultados positivos em amostra concentrada não autorizam generalização nacional.
11. Estados com menos de 30 respostas não deverão receber conclusão isolada.
12. Participantes fora do Brasil deverão ser analisados separadamente da base nacional.
13. Evidência comportamental posterior poderá confirmar ou contrariar a pesquisa declarativa.
