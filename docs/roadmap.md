...
7. Home, início protegido, compreensão inicial, Tela Hoje e Mapa estruturados e validados;
...
→ pausa, limitação ou encerramento
→ mensuração válida
→ reconciliação
→ relatório agregado
```
...
```text
visão geral desktop
→ atribuição candidata e autorrelato desktop
→ visão geral móvel
