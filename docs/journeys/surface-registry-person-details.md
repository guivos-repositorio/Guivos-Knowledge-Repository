...
| GKR-SURF-PER-010 | GKR-UX-D5-C1-001; autoridade funcional PAS-001-OBJ-VIEW-001; sem SVG | 1.0.0 contratual | `TRN-008` a partir de PER-008 | compreender, organizar e controlar objetivos sem julgamento ou mutação automática | `TRN-009` para PER-008; demais handoffs diretos não contratados | objetivos, estados, prioridades, critérios, marcos, progresso/evidências legítimas, conflitos, histórico, permissões e `0..n domain_link` | autenticação; autoridade da Pessoa; proteção proporcional a conteúdo sensível | retornar por `TRN-009`; revisar/pausar/retomar/retirar conforme contrato funcional | nenhuma | `TRN-008/009` contratadas | sem SVG; sem validação ponta a ponta; handoffs diretos com PER-011/012 não definidos | Área da jornada ≠ dimensão estrutural do Contexto Vivo; domínio não cria Objetivo nem prioridade |
| GKR-SURF-PER-011 | GKR-UX-D5-C1-001; autoridade funcional PAS-001-PP-VIEW-001; sem SVG | 1.0.0 contratual | `TRN-010` a partir de PER-008 | compreender, organizar e controlar movimentos contextuais sem converter sugestão em execução | `TRN-011` para PER-008; demais handoffs diretos não contratados | passos, estados, prontidão, prioridade, dependências, bloqueios, responsabilidade, recorrência, resultados, histórico e `0..n domain_link` | autenticação; decisão consciente; minimização de conteúdo sensível | retornar por `TRN-011`; adiar/pausar/cancelar/substituir conforme contrato funcional | nenhuma | `TRN-010/011` contratadas | sem SVG; sem validação ponta a ponta; handoffs diretos com PER-010/012 não definidos | Área da jornada é contexto semântico; não representa obrigação, urgência ou mérito |
| GKR-SURF-PER-012 | GKR-UX-D5-C1-001; autoridade funcional PAS-001-EC-VIEW-001; sem SVG | 1.0.0 contratual | `TRN-012` a partir de PER-008 | compreender e controlar trajetórias, mudanças, continuidades e interpretações sem score humano | `TRN-013` para PER-008; demais handoffs diretos não contratados | trajetórias, períodos, baselines, direções, mudanças/estabilidade, observações, evidências, confiança, incerteza, contestações, histórico e `0..n domain_link` | autenticação; privacidade por padrão; distinção entre declarado/observado/inferido/confirmado | retornar por `TRN-013`; contestar/corrigir/pausar/revogar conforme contrato funcional | nenhuma | `TRN-012/013` contratadas | sem SVG; sem validação ponta a ponta; sem roda da vida/score | Domínio de Evolução, dimensão do Contexto Vivo e aspecto descritivo da mudança devem permanecer separados |
...
`PER-010`, `PER-011` e `PER-012` tornam explícitas responsabilidades já contratadas no PAS-001 sem convertê-las em telas implementadas. Seus seis handoffs mínimos com Hoje ficam contratados e reversíveis; a ausência de SVG impede promoção para materializado ou validado.

A D5-C1 também impede que o termo histórico `dimensão` seja usado na futura UX como categoria única para Domínio de Evolução, dimensão estrutural do Contexto Vivo e aspecto descritivo da mudança.
