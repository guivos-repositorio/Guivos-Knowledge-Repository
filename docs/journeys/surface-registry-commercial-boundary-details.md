...
A UXA-099 transforma `COM-005` de conjunto materializado sem validação específica em conjunto funcionalmente validado. A alteração não promove `TRN-305`, `TRN-304` ou `TRN-306` e não cria implementação.

## 5. Estado

O detalhamento está `active` como parte integrante do registro. O status aprova o instrumento documental e não altera automaticamente maturidade de transições ou jornadas.
