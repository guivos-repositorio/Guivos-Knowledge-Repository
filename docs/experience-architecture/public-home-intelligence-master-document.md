...
### REAL_DATA_REQUIRED — Intelligence

Exige fonte/lastro antes de parecer factual: **capacidade operacional, integração, fonte, indicador, benchmark, precisão, resultado, caso, métrica, disponibilidade e evidência apresentados como reais**.
...
### PROHIBITED_INFERENCE — Intelligence

Não inferir nem comunicar como verdade: **Intelligence = chatbot/LLM/Neo4j; predição como destino; causalidade a partir de correlação; score opaco de pessoa; decisão automatizada em lugar do participante; exposição indevida de contexto pessoal**.
