...
`GPA-006 — Guivos Intelligence 2.0.2` governa a **identidade e a autoridade de produto**.
...
`GPA-006 2.0.2` consolida como responsabilidades funcionais:
...
`GPA-006 2.0.2` reconhece Intelligence Serving como responsabilidade de entregar outputs ao consumidor autorizado na granularidade, momento, canal e forma adequados.
...
`GPA-006 2.0.2` é a autoridade superior de produto do Guivos Intelligence. `GIA-COG-001 v0.1.2` é a Cognitive Reference Architecture vigente, ativa e normativa, subordinada à GIA por `ADR-008`. O **Product Source Lock** vigente é `GKR-INTELLIGENCE-PRODUCT-SOURCELOCK-001 v1.0.3`; a **Home Pública do Intelligence v1** possui arquitetura conceitual em 11 movimentos e Documento Mestre `GKR-UX-HOME-INTELLIGENCE-MASTER-001 v0.2.14`; e o **Source Lock da Home** vigente é `GKR-UX-HOME-INTELLIGENCE-SOURCELOCK-001 v1.1.16`.
...
O estado `active / normative` de `GIA-COG-001` não autoriza por si só Product Engineering, implementação ou produção. O Design da Home Pública é governado separadamente pelo release comum vigente e está liberado para execução externa pela designer.